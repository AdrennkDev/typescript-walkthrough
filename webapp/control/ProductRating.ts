...
export default class ProductRating extends Control {
	...
	_onRate(event: RatingIndicator$LiveChangeEvent): void {
		...
		const sText = resourceBundle.getText("productRatingLabelIndicator", [value, (event.getSource() as RatingIndicator).getMaxValue()]);
		(this.getAggregation("_label") as Label).setText(sText);
